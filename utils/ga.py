...
class GeneticAlgorithm:
    ...
    def initialize_population(self):
        model = NARXModel(input_dim=self.input_dim)
    ...
    def crossover(self, parent1, parent2):
        child = NARXModel(input_dim=self.input_dim)
        with torch.no_grad():
            for p1, p2, c in zip(parent1.parameters(), parent2.parameters(), child.parameters()):
                if np.random.rand() < self.crossover_rate:
                    c.data.copy_(self.two_point_crossover_tensor(p1.data, p2.data))
                else:
    ...
    def select_population(self, offspring):
        ...
        self.population = combined_population[:self.population_size]  # Keep best models

    def run(self, generations):
        for gen in range(generations):
            offspring = []
            for _ in range(self.population_size // 2):  # Generate offspring
